...
export function line_to_wall(grid: TileGrid, x0: number, y0: number, x1: number, y1: number): Array<[number, number]> {
  let dx = Math.abs(x1 - x0);
  let dy = Math.abs(y1 - y0);
  let sx = (x0 < x1) ? 1 : -1;
  let sy = (y0 < y1) ? 1 : -1;
  ...
  while(true) {
    ...
    //if ((x0 === x1) && (y0 === y1)) break;
    //if (points_eq(x0, y0, x1, y1)) break;
    if (grid.at(x0, y0) == TileType.WALL || grid.at(x0, y0) == TileType.DOORCLOSED) break;
    let e2 = 2*err;
    if (e2 > -dy) { err -= dy; x0  += sx; }
    if (e2 < dx) { err += dx; y0  += sy; }
  }
